...
def check_unattended_files():
    ...
    for file in files:
        path = f"{os.path.expandvars('%windir%')}{file}"
...
def checks_writeable_directory_on_path_environment_variable():
    ...
    """
    return [p for p in os.environ['PATH'].split(';') if is_root_dir_writable(p)]
...
def check_well_known_dll_injections(service):
    ...
    for s in service:
        results.extend(
            {'Service': d['service'], 'Associated dll': d['associate_dll']}
            for d in knows_dlls
            if d['service'] in s.name.lower()
            and not os.path.exists(d['associate_dll'])
        )
    return results
